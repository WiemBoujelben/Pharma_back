...
class AccountCreateTransaction extends _Transaction.default {
  ...
  setECDSAKeyWithAlias(key) {
    ...
    if (!(key instanceof _PrivateKey.default) || key.type !== "secp256k1") {
      throw new Error("'key' must be an ECDSA private key when 'aliasKey' is not provided.");
    }
  ...
  setKeyWithAlias(key, aliasKey) {
    this.setKeyWithoutAlias(key);
    if (!(aliasKey instanceof _PrivateKey.default) || aliasKey.type !== "secp256k1") {
      throw new Error("'aliasKey' must be an ECDSA private key when provided.");
    }
