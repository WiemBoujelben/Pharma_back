...
class NodeChannel extends _Channel.default {
  ...
  parseAddress(address) {
    ...
    if (!ip || !port) {
      throw new Error("Invalid address format. Expected format: 'IP:Port'");
    }
    return {
      ip,
      port
  ...
  async _initializeClient() {
    ...
    if (this.nodePort === "50212") {
      ...
      security = _grpcJs.credentials.createSsl(certificate);
    } else {
      security = _grpcJs.credentials.createInsecure();
    }
    this._client = new _grpcJs.Client(this.address, security, options);
      ...
      this._initializeClient().then(() => {
        ...
        this._client?.waitForReady(deadline, err => {
          if (err) {
            ...
            _ClientConstants.ALL_NETWORK_IPS[`${this.nodeIp}:`]));
          } else {
            this._client?.makeUnaryRequest(`/proto.${serviceName}/${method.name}`, value => value, value => value, Buffer.from(requestData), (e, r) => {
              callback(e, r);
            });
          }
        });
